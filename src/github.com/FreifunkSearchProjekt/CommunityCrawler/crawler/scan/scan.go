...
func (s *scanner) scan() (error, map[int]string) {
	...
	for {
		...
		if net := packet.NetworkLayer(); net == nil {
			...
			open[len(open)] = tcp.SrcPort.String()
...
func Scan(ips map[int]net.IP) map[string]map[int]string {
	router, err := routing.New()
	if err != nil {
		log.Fatal("routing error:", err)
	}

	openForIP := make(map[string]map[int]string)
	for _, ip := range ips {
		if ip = ip.To4(); ip == nil {
			log.Printf("non-ipv4 target: %q", ip.String())
			continue
		}
